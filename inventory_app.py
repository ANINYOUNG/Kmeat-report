# inventory_app.py (Google Drive API - .xlsx 직접 로딩 버전 - 요일 표시 및 용어 변경)
    ...
    try:
        if os.path.exists(SERVICE_ACCOUNT_FILE):
            ...
            if not IS_CLOUD_ENVIRONMENT:
                if 'sidebar_error_displayed' not in st.session_state:
                    st.sidebar.warning(f"경고: 서비스 계정 키 파일을 찾을 수 없습니다: {SERVICE_ACCOUNT_FILE}. Google Drive 연동 기능이 제한될 수 있습니다.")
                    st.session_state.sidebar_error_displayed = True
...
def download_excel_from_drive(file_id, file_name_for_error_msg=""):
    if drive_service is None:
        if 'drive_service_none_in_download' not in st.session_state:
            st.sidebar.error(f"오류: 파일 다운로드 시도 중 Google Drive 서비스가 초기화되지 않았습니다 ({file_name_for_error_msg}).")
