...
def load_sales_history_and_filter_3m(_drive_service, file_id_sales, sheet_name, num_months=3): # num_months는 월평균 계산시 사용
    """
    ...
    try:
        ...
        # --- 매출 분석 기간 로직 수정 (사용자 요청 반영: 마지막 날짜로부터 90일) ---
        max_sales_date = df[SALES_DATE_COL].max() # 데이터의 가장 마지막 매출일
        if pd.isna(max_sales_date):
            st.warning(f"매출 데이터 (ID: {file_id_sales}, 시트: {sheet_name})에서 유효한 최대 매출일자를 찾을 수 없습니다.")
            return pd.DataFrame()

        # 분석 종료일: 데이터의 가장 마지막 매출일
        end_date_of_analysis_period = max_sales_date 
        
        # 분석 시작일: 분석 종료일로부터 89일 이전 (총 90일 기간)
...
num_months_to_analyze = 3 # 월평균 계산 시 나누는 값 (90일 / 3 = 30일 평균)
df_total_sales_90d = load_sales_history_and_filter_3m(drive_service, SALES_FILE_ID, SALES_DATA_SHEET_NAME, num_months=num_months_to_analyze) # 함수명은 유지, 내부 로직 변경됨
...
if df_total_sales_90d.empty or df_current_stock.empty:
    ...
    # '월평균 출고량'은 90일간의 총 출고량을 num_months_to_analyze(3)으로 나눈 값
    ...
    # '월평균 출고일수'도 90일간의 총 출고일수를 num_months_to_analyze(3)으로 나눈 값
    ...
    st.dataframe(
        ...
        .map(highlight_refrigerated_product_name, subset=['상품명']) # "냉장" 상품명 강조 적용
